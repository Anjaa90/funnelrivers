<?php
$menuactive = basename($_SERVER['PHP_SELF']);
switch ($menuactive) {
    case "index.php":
        $title = 'Digital Marketing Services By funnelrivers| Navi Mumbai';
        $description = 'funnelrivers enhances your digital presence with web development, marketing, and growth-focused solutions for real results.';
        break;

    case "testimonial.php":
        $title = 'funnelrivers Client Testimonials – Proven Results In Marketing';
        $description = 'funnelrivers enhances your digital presence with web development, marketing, and growth-focused solutions for real results.';
        break;

    case "our-clients.php":
        $title = 'funnelrivers – Digital Success for Businesses of All Sizes';
        $description = 'From SMEs to enterprises, we deliver top-notch branding, digital marketing, web development, and e-commerce solutions for impactful results.';
        break;

    case "career.php":
        $title = 'funnelrivers Careers – Where Talent Meets Opportunity';
        $description = 'Discover exciting roles in digital marketing, web development, and branding. Thrive in a dynamic environment dedicated to innovation and growth.';
        break;

    case "seo.php":
        $title = 'Expert SEO Services to Boost Your Online Visibility';
        $description = 'Boost your rankings and traffic with funnelrivers\'s tailored SEO strategies for improved search performance.';
        break;

    case "smo.php":
        $title = 'funnelrivers SMO | Enhance Your Social Media Presence';
        $description = 'Grow your social media following and engagement with funnelrivers’s customized SMO services.';
        break;

    case "marketing-automation.php":
        $title = 'Marketing Automation by funnelrivers – Transform Your Strategy';
        $description = 'Optimize workflows and drive conversions with funnelrivers’s advanced marketing automation solutions.';
        break;

    case "google-ads.php":
        $title = 'Google Ads Services by funnelrivers – Increase Sales & Leads';
        $description = 'Achieve higher conversion rates with funnelrivers’s customized Google Ads campaigns. Drive traffic, boost sales, and track performance with precision.';
        break;

    case "facebook-ads.php":
        $title = 'Expert Facebook Ads Services for Your Business – funnelrivers';
        $description = 'Reach the right audience and boost your digital marketing results with funnelrivers’s targeted Facebook Ads strategies for improved ROI and engagement.';
        break;

    case "youtube-ads.php":
        $title = 'funnelrivers YouTube Ads – Increase Engagement & Conversions';
        $description = 'Capture your audience’s attention with compelling YouTube Ads. funnelrivers optimizes campaigns to drive traffic, generate leads, and improve ROI.';
        break;

    case "X-twitter-ads.php":
        $title = 'Maximize Engagement with funnelrivers’s Twitter Ads Services';
        $description = 'Increase brand visibility and drive traffic with funnelrivers’s expert Twitter Ads campaigns and social media strategies.';
        break;

    case "instagram-ads.php":
        $title = 'Boost Your Brand with funnelrivers’s Instagram Ads Services';
        $description = 'Reach a larger audience and drive conversions with funnelrivers’s expert Instagram Ads campaigns and social media marketing strategies.';
        break;

    case "content-writing.php":
        $title = 'High-Quality Content Writing Services by funnelrivers';
        $description = 'Elevate your brand with compelling content. funnelrivers delivers customized writing services that resonate with your audience and improve SEO.';
        break;

    case "website-development.php":
        $title = 'Website Development by funnelrivers – Design, Build, Optimize';
        $description = 'funnelrivers offers custom website development, e-commerce solutions, and mobile-friendly designs to enhance user experience and drive conversions.';
        break;

    case "graphic-videos.php":
        $title = 'funnelrivers Graphic Design & Video Services – Impactful Visuals';
        $description = 'Boost your marketing efforts with funnelrivers’s professional graphic design and video services. Custom visuals and videos tailored to your brand’s message.';
        break;

    case "resume-making.php":
        $title = 'Professional Resume Making Services – funnelrivers';
        $description = 'Increase your chances of landing your dream job with funnelrivers’s resume making services. Professional resumes that showcase your experience and skills.';
        break;

    case "contact-us.php":
        $title = 'Contact Us | funnelriverss | Start Your Digital Business Today';
        $description = 'Get in touch with funnelrivers for expert digital marketing, web development, and more. Reach out today and start your journey to success.';
        break;

    default:
        $title = 'funnelrivers | Digital Marketing Services';
        $description = 'Explore funnelrivers’s diverse range of services to elevate your digital presence.';
        break;
}
?>
